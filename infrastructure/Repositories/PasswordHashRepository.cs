...
FROM tennis_app.password_hash
JOIN tennis_app.users ON tennis_app.password_hash.user_id = users.id
WHERE email = @email;
";
    ...
    {
        const string sql = $@"
INSERT INTO tennis_app.password_hash (user_id, hash, salt, algorithm)
...
";
    ...
    {
        const string sql = $@"
UPDATE tennis_app.password_hash
