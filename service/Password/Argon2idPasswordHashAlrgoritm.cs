using System.Text;
using Konscious.Security.Cryptography;

namespace service.Password;

public class Argon2idPasswordHashAlrgoritm : PasswordHashAlgorithm
{
    //extends upon PasswordHashAlgoritm
    //Allows us to have an easier way to upgrade our algorithm in the case it becomes broken in the future.
    //PasswordHashAlgorithm and its subclasses are a variation of the strategy design pattern, where AccountService acts as the context.
    
    
    
    public const string Name = "argon2id";

    public override string GetName() => Name;

    public override string HashPassword(string password, string salt)
    {
        using var hashAlgo = new Argon2id(Encoding.UTF8.GetBytes(password))
        {
            Salt = Decode(salt),
            MemorySize = 12288,
            Iterations = 3,
            DegreeOfParallelism = 1,
        };
        return Encode(hashAlgo.GetBytes(256));
    }
    
    public override bool VerifyHashedPassword(string password, string hash, string salt)
    {
        return HashPassword(password, salt).SequenceEqual(hash);
    }
}