...
public class CreateShotUI : PageTest
{ [TestCase("Forced Error", "Backhand Return",  "Long","Down The Line", "Yellow")]
    ...
    public async Task ShotCanSuccessfullyBeRegistered(string shotClassification, string shotType,  string shotDestination,string shotDirection, string playerPosition)
    {
        //ARRANGE
        ...
        await using (var conn = await Helper.DataSource.OpenConnectionAsync())
        {
            //Created shot should be visible in db
